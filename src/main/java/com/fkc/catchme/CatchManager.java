...
public class CatchManager {
    ...
    public void forceRelease(UUID playerId) {
        // PHASE 1: figure out what needs to change, and fully mutate the
        // map BEFORE touching any entity. removePassenger() synchronously
        // fires EntityDismountEvent, which re-enters this class via
        // CatchListener and would otherwise mutate activeCarries while
        // we're still iterating/removing from it here — that reentrancy is
        // exactly what caused the ConcurrentModificationException.
        ...
        // PHASE 2: map is now fully consistent, safe to touch entities and
        // let any events (and their reentrant handlers) fire.
    ...
    public void forceReleaseEntity(UUID entityId) {
        // Same two-phase approach as forceRelease() above, for the same reason.
